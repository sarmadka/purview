...
const callbackResolves: { [key: string]: (retVal: unknown) => void } = {}
...
function addWebSocketHandlers(state: WebSocketState, location: Location): void {
  ...
  ws.addEventListener("message", messageEvent => {
    ...
    switch (message.type) {
      ...
      case "eventResponse":
        const callbackResolve = callbackResolves[message.eventCallbackResolveId]
        if (callbackResolve) {
          callbackResolve(message.retVal)
          delete callbackResolves[message.eventCallbackResolveId]
        }
        break
    }
  })
    ...
    event => {
      ...
      while (triggerElem) {
        ...
        const message: EventMessage = {
          ...
          callbackResolveId: "", // TODO: switch to null
        }
        const callbackResolve = (target as any).callbackResolve
        if (callbackResolve) {
          ;(target as any).callbackResolve = undefined
          const id = nanoid()
          callbackResolves[id] = callbackResolve
          message.callbackResolveId = id
        }
